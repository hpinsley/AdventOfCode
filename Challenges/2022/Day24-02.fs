...
let parseGridIntoState (grid:Cell[,]) : State =
    ...
    let start_rc = findEmptyColumnInRow grid 0
    let start = { row = fst start_rc; col = snd start_rc; t = 0 }
...
let solveState (state:State) =
            ...
            let neighbors = [(0,0)
                             ...
                             (0,-1); (0,1)]
                                ...
                                |> List.filter (fun (r,c) -> 
                                                    (
                                                         ((r,c) = state.finish)
                                                      || ((r,c) = (state.start.row, state.start.col))
                                                      || ((r > 0 && r < state.rows - 1) &&
                                                         (c > 0 && c < state.cols - 1)))
                                               )
    ...
    *)
    let path = aStar state.start isGoal getNeighbors dist h (Some notifyEnqueue) (Some notifyDequeue)
    path
